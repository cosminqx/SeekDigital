  ...
  return (
    <div className="border-b border-[#222222]">
      ...
      <RevealSection className="mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8">
        <div className="rounded-sm border border-[#222222] bg-white/[0.03] px-5 py-4 text-sm text-muted">
          ...
          {portfolioProjects.map((project) => (
            <RevealSection key={project.title}>
              <Card className="overflow-hidden bg-card">
                <div className="relative aspect-[4/3] border-b border-[#222222] bg-[linear-gradient(135deg,#111111_0%,#171717_35%,#0f0f0f_100%)]">
                  <Image
                  ...
                  <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-4">
                    <p className="max-w-[220px] text-sm uppercase tracking-[0.18em] text-parchment/80">
                      {project.status}
                    </p>
                    <span className="rounded-[2px] border border-[#222222] bg-black/70 px-2 py-1 text-[10px] uppercase tracking-[0.2em] text-muted">
                      Preview
                    </span>
                  </div>
                </div>
                <div className="p-6">
                  <h3 className="font-display text-2xl font-bold uppercase tracking-[-0.03em] text-parchment">
                    {project.title}
                  </h3>
          ...
          {testimonials.map((item) => (
            <Card key={item.name} className="p-6">
              <div className="flex items-center gap-4">
                <div className="flex h-14 w-14 items-center justify-center rounded-full border border-[#222222] bg-white/[0.04] font-display text-lg font-bold text-accent">
                  {item.initials}
                </div>
                <div>
                  <p className="font-display text-lg font-bold uppercase tracking-[-0.03em] text-parchment">{item.name}</p>
      ...
      <RevealSection className="mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8 lg:pb-40">
        <div className="flex flex-col items-start justify-between gap-6 rounded-sm border border-[#222222] bg-white/[0.03] p-6 md:flex-row md:items-center">
          <div>
            <p className="font-display text-xs uppercase tracking-[0.24em] text-accent">Next step</p>
            <h3 className="mt-3 font-display text-2xl font-bold uppercase tracking-[-0.03em] text-parchment">
